...
add_github_actions_citation <- function(
  open = FALSE,
  overwrite = FALSE,
  quiet = FALSE
) {
  ...
  if (file.exists(path) && !overwrite) {
    if (!open) {
      stop(
        "An '.github/workflows/update-citation-cff.yaml' file is already ",
  ...
  if (!quiet) {
    ui_done(paste0(
      "Writing {ui_value('.github/workflows/",
  ...
  if (!quiet) {
    ui_done("Writing {ui_value('CITATION.cff')} file")
  }
