...
add_lifecycle_badge <- function(lifecycle = "experimental", quiet = FALSE) {
  ...
  if (
    ...
    stop(
      "Argument 'lifecycle' must be of among 'experimental', 'stable', ",
  ...
  href <- paste0("https://lifecycle.r-lib.org/articles/stages.html#", lifecycle)
  ...
  if (!quiet) {
    ui_done(paste0(
      "Adding {ui_field('Lifecycle')} badge to ",
      "{ui_value('README.Rmd')}"
    ))
  }
