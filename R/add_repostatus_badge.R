...
add_repostatus_badge <- function(status = "concept", quiet = FALSE) {
  ...
  if (
    ...
    stop(
      "Argument 'status' must be of among 'concept', 'wip', 'suspended', ",
  ...
  alt <- paste0("Project Status: ", status)
  href <- paste0("https://www.repostatus.org/#", tolower(status))
  img <- paste0(
    "https://www.repostatus.org/badges/latest/",
    tolower(status),
    ".svg"
  )
  ...
  if (!quiet) {
    ui_done(paste0(
      "Adding {ui_field('Repo Status')} badge to ",
      "{ui_value('README.Rmd')}"
    ))
  }
