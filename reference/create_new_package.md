...
3.  Run `create_new_package()` to create a new package structure (and
...
directly with the function `create_new_package()` (and others). But if
you create a lot a projects (packages and/or compendiums) it can be
...
of the `create_new_package()`) or permanently by re-running
[`set_credentials()`](https://frbcesab.github.io/rcompendium/reference/set_credentials.md).
