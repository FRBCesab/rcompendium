...
**N.B.** If you change `Title`, `Description`, `Version`, and `Authors` values, do not forget to
also change the `inst/CITATION` file and the `README.Rmd`.
...
4. Update required packages in `DESCRIPTION` with `rcompendium::add_dependencies()`
...
You can also provide data with your package. See `?usethis::use_data`. We 
strongly recommend that you read the book [R packages](https://r-pkgs.org/) by 
