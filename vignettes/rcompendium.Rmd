...
These folders and the file `make.R` will be added to the `.Rbuildignore`
so that some parts of the compendium (essentially the R functions) can be used
as an R package (with `devtools::load_all()`).
