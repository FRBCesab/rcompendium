...
To add a `Dockerfile` for your compendium, use `dockerfile = TRUE` in 
...
You can customize this `Dockerfile` (e.g. system dependencies) and use a different 
default Docker image (i.e. `tidyverse`, `verse`, `geospatial`, etc.). 
...
Once the project is ready to be shared, user must build the Docker image by
